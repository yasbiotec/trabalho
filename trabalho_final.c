...
void InitGame(void)
{
    ...
    fruit.color = SKYBLUE;
    fruit.active = false;
    fruit.special = false; // Inicia como uma fruta normal
}
    ...
    if (!gameOver)
    {
        ...
        if (!pause)
        {
            ...
            if (!fruit.active)
            {
                ...
                //Determina se a fruta tem cor
                fruit.special = GetRandomValue(0, 2) == 0; // 33% chance of being special
            ...
            {
                if (fruit.special){
                    // determina  o poder da fruta e a cor dela
                    fruit.color= RED;
                    pontos += 15;
